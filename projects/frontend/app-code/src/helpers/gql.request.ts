...
export interface IConversationWithoutMessages extends Omit<IConversation, 'messages'> {}

export const SEND_NEW_MESSAGE = gql`
  mutation SendNewMessage($conversationId: String!, $senderId: String!, $content: String!) {
    messageCreate(conversationId: $conversationId, senderId: $senderId, content: $content) {
      id
      conversationId
      content
      createdAt
...
const NEW_MESSAGE_SENT_SUBSCRIPTION = gql`
  subscription OnNewMessageSent($conversationId: String!) {
...
export const getConversationDetails = (id: string) => {
  ...
  return {
    conversation: data?.conversationById as Nullable<IConversationWithoutMessages>,
    loading,
    error,
    refetch,
  };
...
export const sendNewMessage = (conversationId: string, senderId: string, content: string) => {
  const [createMessage, { data, loading, error }] = useMutation(SEND_NEW_MESSAGE, {
...
export const subscribeToConversation = (conversationId: string) => {
  const { data, loading, error } = useSubscription(NEW_MESSAGE_SENT_SUBSCRIPTION, {
    variables: { conversationId },
    fetchPolicy: 'no-cache',
  });
...
export const sendNewMessageAsync = (conversationId: string, senderId: string, content: string) =>
  mainGqlClient
    .mutate({
